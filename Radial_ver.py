    ...
    def delive_cicle(self):
        ...
        if self.wh_mass[self.wh_numb].products_num() == 0:
            if self.wh_mass[self.wh_numb].extornot:
                ...
                self.wh_mass[self.wh_numb].radius += 100 #шаг
...
#можно задавать распределение по складам
...
for turn_simul in range(TURNS,0,-1):
    if turn_simul % 313 == 0 or turn_simul == TURNS:
        print(TURNS - turn_simul, " ", SCORE)
        massforgraths.append(str(TURNS-turn_simul) + " " + str(SCORE))
