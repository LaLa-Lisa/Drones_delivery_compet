...
class Drone(object):
    ...
    def delive_cicle(self):
        ...
        maxi = self.wh_numb
        rasto = distance(self.wh_mass[maxi].orders[way[len(way)-1]].point, self.wh_mass[maxi].point)
        for i in range(len(self.wh_mass)):
            if distance(self.wh_mass[i].point, self.wh_mass[maxi].point) < rasto and self.wh_mass[i].products_num() != 0:
                maxi = i
                rasto = distance(self.wh_mass[i].point, self.wh_mass[maxi].point)
        self.wh_numb = maxi
        pass
    ...
    def f_way(self, x):
        ...
        maxi = self.wh_numb
        rasto = distance(self.wh_mass[maxi].orders[x[len(x)-1]].point, self.wh_mass[maxi].point)
        for i in range(len(self.wh_mass)):
            if distance(self.wh_mass[i].point, self.wh_mass[maxi].point) < rasto and self.wh_mass[i].products_num() != 0:
                maxi = i
                rasto = distance(self.wh_mass[i].point, self.wh_mass[maxi].point)
        sum += math.ceil(distance(self.wh_mass[maxi].point, self.wh_mass[self.wh_numb].orders[x[m - 1]].point))
